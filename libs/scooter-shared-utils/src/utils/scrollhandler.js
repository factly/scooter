export const scrollHandler = ({ wrapperRef, itemIndex, groupIndex }) => {
  if (
    isNaN(itemIndex) || isNaN(groupIndex) ||
    itemIndex === -1 || groupIndex === -1 ||
    itemIndex === undefined || groupIndex === undefined ||
    !wrapperRef.current
  ) {
  ...
  if (selectedItem.offsetTop + itemHeight > scrollPosition + parentItem.clientHeight) {
    scrollPosition = selectedItem.offsetTop - parentItem.clientHeight + itemHeight;
